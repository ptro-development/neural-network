...
def init_tests(runs=36):
    ...
    tests.append({
        "count": runs,
        "start": ["START", " 12. Starting from", "network without change, a lower bound"],
        "stop": ["END", " 12. Starting from", "network without change, a lower bound"],
    })
    #return tests
    tests.append({
            "count": runs,
...
def print_errors_and_training_time(results, converted_results):
    ...
    for index, result in enumerate(results):
        if result["test"].replace(',','').find("network without change") > 0:
            indexes.append(index)
            values.append(transposed_results[2][0][index])
    for i, index_i in enumerate(indexes):
        for index_j in xrange(0, len(transposed_results[2])):
            transposed_results[2][index_j][index_i] -= values[i]
        ...
        for line in lines:
            if all([line.find(i) > -1 for i in test["start"]]):
                ...
                if not run_results.has_key("test"):
                    if test_line.split(" ", 5)[4] == "":
                        tmp = test_line.split(" ", 11)
                        tmp.remove("")
                        full_test_name = tmp[-1].rsplit(",", 1)[-2]
                    else:
